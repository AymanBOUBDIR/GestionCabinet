...
public class PatientDao implements IPatientDao {
    ...
    public void update(Patient patient) throws SQLException {
        Connection connection = DBConnection.getConnection();
        PreparedStatement pstm =DBConnection.getConnection().prepareStatement("UPDATE  PATIENTS SET NOM=?,PRENOM=?,TEL=? WHERE ID_PATIENT=?");
        pstm.setString(1,patient.getNom());
        pstm.setString(2,patient.getPrenom());
        pstm.setString(3,patient.getTel());
    ...
    public void delete(Patient patient) throws SQLException {
        Connection connection = DBConnection.getConnection();
        PreparedStatement pstm =DBConnection.getConnection().prepareStatement("DELETE FROM PATIENTS WHERE ID_PATIENT=?");
    ...
    public List<Patient> findAll() throws SQLException {
        Connection connection = DBConnection.getConnection();
        PreparedStatement pstm = DBConnection.getConnection().prepareStatement("SELECT * FROM PATIENTS");
    ...
    public Patient findById(Long id) throws SQLException {
        Connection connection = DBConnection.getConnection();
        PreparedStatement pstm = DBConnection.getConnection().prepareStatement("SELECT * FROM PATIENTS WHERE ID_PATIENT=?");
        pstm.setLong(1,id);
        ResultSet rs=pstm.executeQuery();
        Patient patient = new Patient();
        if(rs.next()){
    ...
    @Override
    public List<Patient> StringQuery(String query) {
        Connection connection = DBConnection.getConnection();
        try {
